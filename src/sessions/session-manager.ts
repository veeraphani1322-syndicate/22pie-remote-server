...
export interface SessionView {
  ...
  permissions: Array<"SCREEN_VIEW" | "MOUSE_CONTROL" | "KEYBOARD_CONTROL">;
  requestedAt: string;
  acceptedAt?: string;
  connectedAt?: string;
  endedAt?: string;
...
export class SessionManager {
  ...
  fromAgent(deviceId: string, message: AgentMessage): void {
    ...
    if (message.type === "trust_grant" && (session.status === "requested" || (["MOUSE_CONTROL", "KEYBOARD_CONTROL"].includes(message.permission) && ["accepted", "connecting", "connected"].includes(session.status)))) {
    ...
    if (message.type === "keyboard_control_accept" && ["accepted", "connecting", "connected"].includes(session.status)) {
      if (!session.permissions.includes("KEYBOARD_CONTROL")) session.permissions.push("KEYBOARD_CONTROL");
      send(session.viewer, { type: "keyboard_control_authorized", sessionId: session.sessionId });
      const trusted = this.trustedAccess.has(deviceId, session.userId, "KEYBOARD_CONTROL", this.devices.publicKeyFor(deviceId));
      this.audit({ event: trusted ? "KEYBOARD_CONTROL_TRUSTED" : "KEYBOARD_CONTROL_ALLOWED_ONCE", sessionId: session.sessionId, deviceId, userId: session.userId }, "Keyboard control authorized");
      return;
    }
    if (message.type === "keyboard_control_reject") {
      session.permissions = session.permissions.filter((permission) => permission !== "KEYBOARD_CONTROL");
      send(session.viewer, { type: "keyboard_control_rejected", sessionId: session.sessionId, reason: message.reason ?? "denied_by_remote_user" });
      this.audit({ event: "KEYBOARD_CONTROL_DENIED", sessionId: session.sessionId, deviceId, userId: session.userId }, "Keyboard control denied");
      return;
    }
  ...
  requestKeyboardControl(sessionId: string, userId: string, viewerName: string): SessionView | undefined {
    const session = this.sessions.get(sessionId);
    if (!session || session.userId !== userId || !["accepted", "connecting", "connected"].includes(session.status)) return undefined;
    if (session.permissions.includes("KEYBOARD_CONTROL")) {
      send(this.devices.socketFor(session.deviceId), { type: "keyboard_control_enabled", sessionId });
      send(session.viewer, { type: "keyboard_control_authorized", sessionId });
      return this.view(session);
    }
    const trusted = this.trustedAccess.has(session.deviceId, userId, "KEYBOARD_CONTROL", this.devices.publicKeyFor(session.deviceId));
    const agent = this.devices.socketFor(session.deviceId);
    if (!agent) return undefined;
    this.audit({ event: "KEYBOARD_CONTROL_REQUESTED", sessionId, deviceId: session.deviceId, userId }, "Keyboard control requested");
    send(agent, { type: "keyboard_control_requested", sessionId, viewerUserId: userId, viewerName, trusted });
    return this.view(session);
  }

  disableKeyboardControl(sessionId: string, userId: string): SessionView | undefined {
    const session = this.sessions.get(sessionId);
    if (!session || session.userId !== userId) return undefined;
    send(this.devices.socketFor(session.deviceId), { type: "keyboard_control_disabled", sessionId });
    send(session.viewer, { type: "keyboard_control_disabled", sessionId });
    this.audit({ event: "KEYBOARD_CONTROL_DISABLED", sessionId, deviceId: session.deviceId, userId }, "Keyboard control disabled");
    return this.view(session);
  }
