  ...
  return async (app) => {
    app.get("/devices", async (request, reply) => {
      ...
      const devices = deviceManager.list(user.userId).map((device) => {
        ...
        return {
          ...device,
          trustedScreenAccess: Boolean(screenTrust), trustedMouseControl: Boolean(mouseTrust), trustedKeyboardControl: Boolean(keyboardTrust),
    ...
    app.get("/devices/:deviceId", async (request, reply) => {
      ...
      return { ...device, trustedScreenAccess: trustedAccess.has(device.deviceId, user.userId, "SCREEN_VIEW", deviceManager.publicKeyFor(device.deviceId)), trustedMouseControl: trustedAccess.has(device.deviceId, user.userId, "MOUSE_CONTROL", deviceManager.publicKeyFor(device.deviceId)), trustedKeyboardControl: trustedAccess.has(device.deviceId, user.userId, "KEYBOARD_CONTROL", deviceManager.publicKeyFor(device.deviceId)) };
    });
